...
\begin{frame}[fragile]{Steenrod obstructions to commutativity}
	...
	\phantom{Sq^k \colon}{[\alpha]} \arrow[r, |->] & {\left[(\alpha \otimes \alpha)\Delta_{k-n}(-) \right]}
	\end{tikzcd}
	\end{equation*}
...
\begin{frame}{The relevance of chain level Steenrod squares}
	...
	\begin{itemize}
		...
		\item Are used in persistence (co)homology to extract finner information of data sets (M-M--Tauzin), \pause
		\item Define the nerve of higher categories (M-M), \pause
		\item Can be axiomatically characterized in analogy to the axioms for Steenrod square (M-M), \pause
