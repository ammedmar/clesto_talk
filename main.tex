...
\begin{frame}{Steenrod's obstructions to commutativity}
	...
	\end{equation*}
	has a left $\mathrm{S}_2$-action induced from $T$, and there is a $0$-cycle of interest:
	\begin{equation*}
	\Delta_0 - T \Delta_0.
	\end{equation*}
\end{frame}
...
\begin{frame}[fragile]{Steenrod's obstructions to commutativity}
	...
	More abstractly, if $W(2)$ is the minimal resolution of $\mathbb Z$ by free $\mathbb Z[\mathrm{S}_2]$-modules
	\begin{equation*}
	\begin{tikzcd}[column sep=normal]
	\mathbb{Z}[\mathrm C_r]\{e_0\} & \arrow[l, "1-T"'] \mathbb{Z}[\mathrm C_r]\{e_1\} &[-5pt] \arrow[l, "T+1"']
	\mathbb{Z}[\mathrm C_r]\{e_2\} & \arrow[l, "1-T"'] \cdots
	\end{tikzcd}
	\end{equation*}
...
\begin{frame}[fragile]{Steenrod obstructions to commutativity}
	...
	\begin{equation*}
	\begin{tikzcd}
	Hom\left(C_\bullet \otimes C_\bullet, \F_2 \right)^{\mathrm{S}_2} \arrow[r] &
	Hom\left(W(2) \otimes C_\bullet, \F_2 \right)^{\mathrm{S}_2}
	\end{tikzcd}
	\end{equation*}
\end{frame}
%%%%%%%%%%%%%
\begin{frame}[fragile]{Steenrod obstructions to commutativity}
	...
	\begin{tikzcd}
	Hom\left(C_\bullet \otimes C_\bullet, \F_2 \right)^{\mathrm{S}_2} \arrow[r] &
	Hom\left(W(2) \otimes C_\bullet, \F_2 \right)^{\mathrm{S}_2} \arrow[d] \\
	\left(C^\bullet \otimes C^\bullet\right)^{\mathrm{S}_2} \arrow[u] &
	Hom\left(W(2)_{\mathrm{S}_2} \otimes C_\bullet, \F_2 \right)
	\end{tikzcd}
	\end{equation*}
\end{frame}
%%%%%%%%%%%%%
\begin{frame}[fragile]{Steenrod obstructions to commutativity}
	...
	Hom\left(W(2) \otimes C_\bullet, \F_2 \right)^{\mathrm{S}_2} \arrow[d] \\
	\left(C^\bullet \otimes C^\bullet\right)^{\mathrm{S}_2} \arrow[u]&
	Hom\left(W(2)_{\mathrm{S}_2} \otimes C_\bullet, \F_2 \right) \arrow[d] \\
	C^\bullet \arrow[u, "Double"] \arrow[r, dashed]&
	Hom\left(W(2)_{\mathrm{S}_2}, C^\bullet\right) \\
	\end{tikzcd}
	\end{equation*}
	...
	C^\bullet \otimes W(2)_{\mathrm{S}_2} \arrow[r] &[-10pt] C^\bullet \\
	...
	\textcolor{pblue}{Take away:} The mod-2 homology of $\mathrm{S}_2$ defines operations on the mod-2 cohomology of spaces.
\end{frame}
...
\begin{frame}{Higher diagonals and the homology of $\mathrm{S}_p$}
	What do we need to generalize the construction of Steenrod squares?
	
	\pause \vspace{10pt}
	
	\begin{enumerate}
		\item Construct $\mathrm{S}_p$-equivariant maps
		\begin{equation*}
		E\mathrm{S}_p \otimes C_\bullet \to C_\bullet^{\otimes p}
		\end{equation*}
		for $E\mathrm{S}_p$ a resolution of $\mathbb Z$ by free $\mathbb Z[\mathrm{S}_p]$-modules.
		\vspace*{10pt} \pause
		\item Identify chains in $E\mathrm{S}_p$ whose orbits in $(E\mathrm{S}_p)_{\mathrm{S}_p} = B\mathrm{S}_p$ represent mod-$p$ homology classes.
	\end{enumerate}

	\vspace*{10pt}\pause

	The first step is systematized by the use of operads, which we describe next, and the second is simplified by the group theoretical fact that the mop-$p$ homology of $\mathrm{S}_p$ is detected by any cyclic subgroup of order $p$.
\end{frame}
...
\begin{frame}[fragile]{Canonical example}
	...
	\begin{itemize}
		\item A left action of $\mathrm{S}_r$ on $End^C(r)$,
...
\begin{frame}[fragile]{Coalgebras}
	...
	\begin{definition}
		An $E_\infty$-operad $\mathcal R$ is one with each $\mathcal R(r)$ a resolution of $\mathbb Z$ by free $\mathbb Z[\mathrm{S}_r]$-modules
	\end{definition}
\end{frame}


\begin{frame}[fragile]{Steenrod-Adem structures}
	Now we use the fact that the mod-$p$ homology of $\mathrm{S}_p$ is detected by the inclusion $\langle \rho\ |\ \rho^p\rangle = \mathrm{C}_p \to \mathrm{S}_p$.
	
	\pause \vspace*{10pt}
	
	Let $\mathcal W(r)$ be the minimal resolution of $\mathbb{Z}$ by $\mathbb{Z}[\mathrm C_r]$ modules:
	\begin{equation*}
	\begin{tikzcd}[column sep=normal]
	\mathbb{Z}[\mathrm C_r]\{e_0\} & \arrow[l, "\rho-1"'] \mathbb{Z}[\mathrm C_r]\{e_1\} &[-5pt] \arrow[l, "N"']
	\mathbb{Z}[\mathrm C_r]\{e_2\} & \arrow[l, "\rho-1"'] \cdots
	\end{tikzcd}
	\end{equation*}
